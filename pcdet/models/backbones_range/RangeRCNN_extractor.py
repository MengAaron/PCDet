...
class DRBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(out_channels, out_channels, kernel_size=3, padding=1, bias=False, dilation=2),
            nn.BatchNorm2d(out_channels),
            nn.ReLU(inplace=True))
        self.conv2 = nn.Sequential(
            nn.Conv2d(out_channels, out_channels, kernel_size=3, padding=1, bias=False, dilation=3),
            nn.BatchNorm2d(out_channels),
            nn.ReLU(inplace=True))
...
class RangeRCNNBackbone(nn.Module):
    ...
    def forward(self, batch_dict):
        ...
        up4 = torch.cat([nn.UpsamplingBilinear2d(conv5),conv4],dim=1)
        up4 = self.Up4(up4)
        up3 = torch.cat([nn.UpsamplingBilinear2d(up4),conv3],dim=1)
        up3 = self.Up4(up3)
        up2 = torch.cat([nn.UpsamplingBilinear2d(up3),conv2],dim=1)
        up2 = self.Up4(up2)
